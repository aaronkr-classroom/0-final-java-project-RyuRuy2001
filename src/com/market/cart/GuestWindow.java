...
public class GuestWindow extends JFrame{ 
	public GuestWindow(String title, int x, int y, int width, int height) {
		...
		setIconImage(new ImageIcon("./imgaes/shop.png").getImage());
		...
		ImageIcon imageIcon = new ImageIcon("./image/그림파일 올리면 쓰기"); // 그림 파일 가져오면 그때 쓰기
		...
		JPanel buttonePanel = new JPanel();
		phonePanel.setBounds(0, 500, 1000, 100);
		add(buttonPanel);
		
		JLabel buttonLabel = new JLabel("소핑하기", 
				new ImageIcon("이미지 찾아서 넣기"), JLabel.LEFT); // 이미지 찾아서 넣기
